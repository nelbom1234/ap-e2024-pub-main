...
runEvalIO evalm = do
  ...
  where
    ...
    runEvalIO' r db (Free (TryCatchOp t h)) = do 
      resT <- runEvalIO' r db t
      case resT of
        Right val -> pure $ Right val
        Left _ -> runEvalIO' r db h
    runEvalIO' r db (Free (KvGetOp key k)) = do
      result <- readDB db
      case result of
        Right state -> case lookup key state of
          Just val -> runEvalIO' r db $ k val
          Nothing -> promptForValidKey
            where 
            
            promptForValidKey = do
              input <- prompt $ "Invalid key: " ++ show key ++ ". Enter a replacement: "
              case readVal input of
                Nothing -> do 
                  pure $ Left "Key not in a supported format"
                Just iKey -> runEvalIO' r db (Free (KvGetOp iKey k)) 
        Left err -> pure $ Left err
    runEvalIO' r db (Free (KvPutOp key val m)) = do
      ...
      case result of
        Right state -> do
          ...
          runEvalIO' r db m
        Left err -> pure $ Left err
    runEvalIO' r db (Free (TransactionOp payload m)) = withTempDB $ \tempDB -> do
      copyDB db tempDB
      result <- runEvalIO' r tempDB payload
      case result of
        Right _ -> do
          copyDB tempDB db
          runEvalIO' r db m
