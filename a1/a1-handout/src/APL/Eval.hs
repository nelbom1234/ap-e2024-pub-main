...
printExp (Lambda var e1) = "(" ++ "\\" ++ var ++ " -> " ++ printExp e1 ++ ")"
...
eval env (Apply e1 e2) = 
  case (eval env e1, eval env e2) of
    (Left err, _) -> Left err
    (_, Left err) -> Left err
    (Right (ValFun _env var e), Right (ValInt x)) -> eval _env (Let var (CstInt x) e)
    (Right (ValFun _env var e), Right (ValBool x)) -> eval _env (Let var (CstBool x) e)
    (Right (ValInt x), Right (ValFun _env var e)) -> eval _env (Let var (CstInt x) e)
    (Right (ValBool x), Right (ValFun _env var e)) -> eval _env (Let var (CstBool x) e)
    (Right _, Right _ ) -> Left "No function included"
